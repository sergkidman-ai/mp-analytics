...
CHIP_PATTERNS = [
    ...
    # «б/ч» — ходовое сокращение поставщиков от «без чипа» (подтвердил Сергей 21.08.2026).
    # Разделитель обязателен, а по краям — граница: иначе «б/у» и внутренности кодов
    # («ХБЧ», «SB/CH») попадут в шаблон.
    ("nochip", r"без\s*чипа|без\s*чип\b|(?<![A-Za-zА-Яа-я0-9])б\s*[/.]\s*ч(?![A-Za-zА-Яа-я0-9])"
               r"|w/?o\s*chip|no\s*chip|чип\s*в\s*комплект\w*\s*нет"),
