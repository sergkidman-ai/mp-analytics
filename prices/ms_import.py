...
# Бренды группы ВТТ/Юнитон (ключи `vtt` и `ramis`): лазерка Hi-Black и NetProduct едут от ВТТ,
# струйка MyInk / MyLabel / InkTec / Unijet / ATM — от Юнитона. Хвост кода в МС = бренд, а не
# юрлицо, поэтому таблица общая.
VTT_BRANDS = [(r"hi-?black", "hb"), (r"net\s?product", "np"), (r"my\s?ink", "my"),
...
NAME_SUFFIX = {
    ...
    # ВТТ и Рамис (он же Юнитон) — ОДИН поставщик под разными юрлицами: активное «ООО КПД(ВТТ)»
    # (113 приёмок, последняя 24.08.2026), «ЮНИТОН СПБ» умолк в 2024-м, за самим «Рамисом»
    # документов нет вовсе. Уточнение Сергея 28.08.2026: «Рамис, он же Юнитон, он же ВТТ,
    # у них товары hb идут и другие». Поэтому словарь брендов один на оба ключа.
...
MS_SUPPLIER = {
    ...
    "ramis": 'ООО "КПД"(ВТТ)',      # Рамис=Юнитон=ВТТ, активное юрлицо группы одно
}
