...
# Бренды, которые везут ВТТ и Юнитон (ключи `vtt` и `ramis`). ГРУППЫ ЮРЛИЦ у них РАЗНЫЕ —
# общих контрагентов нет, — но ассортимент пересекается: Hi-Black и NetProduct идут от обоих,
# струйка MyInk / MyLabel / InkTec / Unijet / ATM — от Юнитона. Хвост кода в МС = БРЕНД, а не
# юрлицо, поэтому таблица брендов одна на два ключа, а юрлицо каждый берёт своё.
...
NAME_SUFFIX = {
    ...
    # юрлиц (поправка Сергея 28.08.2026), но общий словарь брендов: «у них товары hb идут
    # и другие» — Hi-Black везут оба, и по имени бренда карточка встаёт на верный код.
