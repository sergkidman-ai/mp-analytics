...
CODE_SUFFIX = {
    ...
    # `SG-…` — Smart Graphics, хвост `sg` (1232 живые карточки из 1246 с таким артикулом,
    # 1242 из 1251 по имени бренда; проверено 04.09.2026). `GF…`/`GD…` — собственная нумерация
    # струйки и чернил БУЛАТА: в МС 203 из 205 `GF…` и 40 из 40 `GD…` стоят на `bt`, и все они
    # названы «… БУЛАТ …». В прайсе имя бренда у этих же позиций пропадает («Струйный картридж
    # HP DesignJet T730/T830 728…»), поэтому NAME_SUFFIX их не ловит — решает префикс артикула.
    # Значение по умолчанию — `bt` (решение Сергея 04.09.2026: «s-Line — sl, e-Line — el,
    # bulat — bt», драм-картриджи Konica/Ricoh/Xerox без марки в имени — «этот Булат»).
    # Раньше умолчания не было намеренно, и на «Новых моделях» падала каждая позиция без
    # марки в наименовании. Проверено по 3889 живым карточкам: до умолчания доезжают 117,
    # из них 111 стоят на `bt`; ни одной `sl` или `el` среди них нет — марки s-Line и e-Line
    # в наименовании написаны всегда. Плата за это — новый бренд Булата молча уедет в `bt`.
    "bulat": [(r"^CET", "ce"), (r"^SG-", "sg"), (r"^G[FD]", "bt"), (r"", "bt")],
...
NAME_SUFFIX = {
    ...
    # Smart Graphics — отдельная марка Булата, а не «БУЛАТ»: 1242 живые карточки из 1251
    # с этим именем собраны как `внешний код + sg`. Стоит выше `булат`, хотя пересечения имён
    # сейчас нет (проверено 04.09.2026: ни одной карточки с обоими словами).
    # e-Line не был описан вовсе: 235 живых карточек МС названы «Тонер-картридж e-Line …»
    # (228 из них на `el`), слова «БУЛАТ» в имени нет — и до правки КАЖДАЯ такая позиция
    # прайса падала на «нет правила аббревиатуры». Дефис необязателен у обеих марок.
    "bulat": [(r"\b7q\b", "7q"), (r"smart\s?graphics", "sg"), (r"grafit|графит", "gf"),
              (r"e-?line", "el"), (r"s-?line", "sl"), (r"булат", "bt")],
