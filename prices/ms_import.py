...
NAME_SUFFIX = {
    ...
    # ВТТ везёт два бренда под общим числовым артикулом (`220095936`, `22013628p`) — по
    # артикулу их не различить вовсе, зато имя бренда всегда стоит в наименовании: «Картридж
    # Hi-Black (HB-CF256X) для HP…» -> `hb` (1291 живая карточка), «NetProduct…» -> `np`
    # (407). В письмах августа 11 строк Hi-Black и 5 NetProduct — покрытие полное. Третий
    # суффикс `my` (10 карточек, плоттерные Canon PFI) правилом НЕ описан: имя бренда в них
    # не встречается вовсе («Картридж для CANON PFI-120Bk»), придумывать признак нельзя.
    "vtt": [(r"hi-?black", "hb"), (r"net\s?product", "np")],
}
