...
SUPPLIERS = {
    ...
    "vtt": (r"^vtt\.txt$", None),
    # Солюшнс принт СПб — второе юрлицо той же марки; отличается от МСК хвостом артикула
    # (`SP…_MSK` — Москва, без хвоста — Питер). Письмо приходит реже (11 писем августа).
    # Наборы (слэш в артикуле) отсекает `blacklist.BRAND_RULES["s_print_spb"]`.
    "s_print_spb": (r"^s_print_spb\.txt$", None),
