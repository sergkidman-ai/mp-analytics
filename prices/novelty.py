...
def namespace(row):
    """Комплект не может собраться из разных типов товара и разных производителей.

    Производителя берём из буквенного префикса артикула (CS-… Cactus, CET… Cet):
    иначе к тонеру Cactus «дособерутся» цвета Cet, и комплекта на деле не будет.
    """
    vendor = VENDOR_RE.match(str(row.get("article") or "").strip())
    return kind(str(row.get("name") or "")), (vendor.group(0).upper() if vendor else "")
