...
Правила (Сергей, 06.08.2026):
  1. промывочная жидкость — не берём;
  2. тонер и чернила по объёму НЕ режем (граница 150 г снята 09.08.2026): годится флакон или
     нет, решает человек по модели — доза должна закрывать полную заправку конкретной модели
     картриджа, а мастер-флакон уезжает в ЧС руками;
  3. тонер и чернила «Материалов для заправки» — только полным цветовым комплектом
     (чёрный + голубой + пурпурный + жёлтый); одинокий голубой не нужен;
  4. то же для картриджей, НО их не хороним: неполный комплект уходит в список
     наблюдения — поставщик может дозавести цвета, и тогда мы комплект заведём;
  5. перезаправляемые картриджи — не берём.
...
# bulk_toner / bulk_ink больше НЕ ставятся (граница 150 г снята 09.08), но пояснения оставлены:
# по ним в журнале лежат строки прошлых прогонов, и отчёты должны их подписывать.
NOVELTY_REASONS = {
    "bulk_toner": ("тонер больше 150 г — правило снято 09.08, строка из старого прогона", False),
    "bulk_ink": ("чернила больше 150 мл — правило снято 09.08, строка из старого прогона", False),
...
# «Тонер» в начале — это порошок во флаконе, но «Тонер-картридж» и его сокращения
# («Тонер К-ж», «Тонер карт.») — обычный картридж, и путать их нельзя: флакон на 100 г
# и картридж на тот же принтер — разные товары.
TONER_RE = re.compile(r"^тонер\b(?!\s*-?\s*(?:картридж|карт\.|к-?ж)\b)", re.I)
INK_RE = re.compile(r"^чернила\b", re.I)
# Чип — не картридж, а деталь к нему; чипы мы не продаём. Якорь на НАЧАЛО обязателен:
# слово «чип» стоит в середине 4422 живых карточек («с чипом», «без чипа») — это обычные
# картриджи, и без якоря правило забраковало бы весь ассортимент. С начала имя «Чип …»
# начинается ровно у 50 карточек МС, и все 50 — действительно чипы.
CHIP_RE = re.compile(r"^чип(?:ы|а|ов)?\b", re.I)
...
# Оговорка `(?!ил)` обязательна: без неё «Чернила Cactus …голубой» читались как ЧЁРНЫЕ —
# слово «чернила» само начинается с «черн». Из-за этого у всех чернил цвет был один и тот же,
# комплект по правилу 4 никогда не собирался, и одинокие цвета уезжали в разбор как новинки.
COLORS = [
    ("BKM", re.compile(r"\bчер[нh](?!ил)\w*\s+матов", re.I)),
    ("BK", re.compile(r"\bчер[нh](?!ил)\w*", re.I)),
...
def kind(name):
    """Что это по названию: чип, тонер-порошок, чернила, промывка, перезаправляемый, картридж."""
    text = _norm(name)
...
def check(row, sets):
    ...
    if what == "refillable":
        return "refillable"
    # Границы по объёму здесь больше НЕТ (решение Сергея 09.08). Она ошибалась в обе стороны:
    # 650 г для TK-3130 — ровно полная заправка, а 100 г для той же тубы — не заправка вовсе.
    # Годится флакон или нет, решает человек по модели: доза должна закрывать полную заправку
    # конкретной модели картриджа. Флакон и правда «мастер» — уедет в ЧС руками.
    own = color(name)
    if own:
