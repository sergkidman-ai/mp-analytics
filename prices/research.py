...
def models_of(row):
    ...
    for match in TOKEN_RE.finditer(text):
        ...
        seen.add(key)
        out.append(code)
    # Хвостовой дубль: у Булата артикул сложен из кусков «тип-вендор-модель-бренд»
    # (`CRT-PNT-TL5120X-GRFT`), и рядом с настоящей моделью TL-5120X вставала призрачная
    # PNT-TL5120X — вендор, приклеенный к модели (замечание Натальи 27.08.2026). Вендорных
    # сокращений у каждого поставщика свои, списком их не догнать, поэтому убираем по форме:
    # модель, оканчивающаяся на другую найденную модель, — это она же с приставкой.
    keys = [norm_model(c) for c in out]
    return [c for c, k in zip(out, keys)
            if not any(o != k and k.endswith(o) for o in keys)]
