...
# Повреждённая упаковка. Поставщик помечает такой товар прямо в наименовании: «ПУ», «П/У»,
# «(Повр. упак.)», «(Поврежд. упак.)», «(Повреждённая упаковка)». Продавать его на площадках
# нельзя, а в разбор он приходит обычной строкой прайса — Екатерина за первые две недели
# забраковала девять таких руками (решение Сергея 16.09.2026: вынести в правило).
# Правило по НАЗВАНИЮ, как `KIND_RULES`, и по той же причине: артикул у повреждённой позиции
# тот же, что у целой, поимённый ЧС забраковал бы заодно нормальный товар.
# Голое «ПУ» считаем меткой только ЗАГЛАВНЫМИ и отдельным токеном — строчное «пу» внутри
# слова меткой не является, поэтому у этого куска регистр НЕ игнорируется.
# Проверено на 350 842 строках прайсов: 21 попадание, ложных нет.
DAMAGED_ANY = re.compile(r"повр\w*[\s.]*упак|\bп\s*/\s*у\b", re.IGNORECASE)
DAMAGED_ABBR = re.compile(r"(?<![A-ZА-ЯЁ0-9])ПУ(?![A-ZА-ЯЁa-zа-яё0-9])")
DAMAGED_WHY = "повреждённая упаковка — не продаём (решение Сергея 16.09.2026)"


def in_damaged(name):
    """Название помечено повреждённой упаковкой? -> причина или None."""
    text = str(name or "")
    return DAMAGED_WHY if (DAMAGED_ANY.search(text) or DAMAGED_ABBR.search(text)) else None
...
def mark(skipped, black, reasons=("not_found", "ambiguous"), supplier_key=None):
    ...
    """
    # Раннего выхода «нечем браковать» здесь нет: правила по названию (`KIND_RULES`,
    # повреждённая упаковка) работают всегда и не зависят ни от загруженного списка
    # артикулов, ни от поставщика.
    hits = 0
    for row in skipped:
