...
4. ~~Даёт ли `client-info` номера счетов~~ — **закрыт 2026-08-02: даёт** (`accounts[]` с `state`/`type`),
   счёт в `.env` не нужен.
5. ~~Есть ли push/webhook о зачислениях~~ — **закрыт 2026-09-07: НЕТ, только поллинг выписки**,
   как у Альфы. В справочнике Sber API нет группы методов событий/подписок; push есть только
   в соседних продуктах — «Плати QR» (уведомления по QR-заказам) и эквайринг/СБП, то есть про
   приём платежей, а не про зачисления на р/с. «Сведения о подписчиках» — это «Корпоративные
   подписки» (безакцептное списание по ЗДА, `/v1/partner-info/advance-acceptances`),
   к подписке на события отношения не имеет. Штатный путь банка — опрос
   `GET /fintech/api/v2/statement/transactions` с дедупликацией по `operationId`.
   Нюанс для частого опроса: `202 STATEMENT_RESPONSE_PROCESSING` = выписка ещё формируется,
   повторить через 1–3 мин; `429 TOO_MANY_REQUESTS` — лимит (числа в документации нет,
   уточнять у менеджера, см. вопрос 6).
