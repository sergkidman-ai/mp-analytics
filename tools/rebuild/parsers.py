...
def parse_cactus():
    ...
    for rn, vals in rows:
        ...
        if not any(vals):
            ...
            out.append(r)
            continue
        r.supplier_code = get(vals, ci['PartNo'])
...
def parse_izi():
    ...
    for rn, vals in rows:
        ...
        if not any(vals):
            ...
            out.append(r)
            continue
        r.supplier_code = get(vals, ci['артикул'])
        r.oem_code = get(vals, ci['аналог'])
...
def parse_sakura_laser():
    ...
    ci = {n: col_index(header, n) for n in (
        ...
        'Длина м', 'Ширина м', 'Высота м', 'Объем м3', 'Вес кг', 'штук в МК')}
    out = []
    for rn, vals in rows:
        ...
        if not any(vals):
            ...
            continue
        r.supplier_code = get(vals, ci['Артикул'])
        r.oem_code = get(vals, ci['Номер оригинального картриджа'])
        r.brand = get(vals, ci['Бренд'])
...
def parse_sakura_inkjet():
    ...
    ci = {n: col_index(header, n) for n in (
        ...
        'Наименование полное', 'Вес коробки кг', 'Кол-во в упаковке', 'Обьем в м3',
    ...
    for rn, vals in rows:
        ...
        if not any(vals):
            ...
            out.append(r)
            continue
        r.supplier_code = get(vals, ci['Код производителя Sakura'])
        r.oem_code = get(vals, ci['Артикул оригинального картриджа'])
        r.brand = get(vals, ci['Бренд'])
...
def parse_rapid():
    ...
    for i, rec in enumerate(recs):
        ...
        r.supplier_code = canon(rec.get('CodeID'))
        r.oem_code = canon(rec.get('CodeShort'))
        ...
        dims = [(n, canon(rec.get(n))) for n in ('GabarityDlina', 'GabarityShirina', 'GabarityVisota')]
        # единица в файле НЕ объявлена → сырьё сохраняем, в мм не пересчитываем
        set_dims(r, 'iu', dims, '')
        # в выгрузке рапида пустое значение записано нулём-строкой '0'
        filled = [v for _, v in dims if v and (num(v) is None or num(v) > 0)]
        if len(filled) == 3:
            r.status = ST_LOADED_NOUNIT
            r.reason = 'единица не объявлена в файле; принадлежность ИУ/МК тоже не объявлена'
        elif filled:
        ...
        out.append(r)
    return out, meta, {'sheet': 'price', 'max_row': len(recs), 'tail': 0, 'header_rows': 0,
