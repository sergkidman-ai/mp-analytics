...
def дописать_оценку(rec, путь=None):
    ...
    data_as_of, отпечаток зрелости, версия оценщика). Дозагрузка данных меняет data_as_of
    и рождает НОВУЮ запись со ссылкой supersedes на прежнюю — прошлое не переписывается.
    """
    п = путь or ОЦЕНКИ
...
def оценить(c, data, today=TODAY, журнал=None, каталог_когорт=None):
    ...
    загр = загрязнения(c, дрейф, пересеч)
    for z in загр:
    ...
    разрешена = есть_контроль and not блок and базис_есть
...
def _собрать(c, вердикт, причины, ch, data_as_of, готовность, base, окно, рез_t, рез_c,
    ...
    ид = _sha(_канон([c.get('hypothesis_id'), c['id'], cv, ch, data_as_of,
                      _отпечаток_зрелости(готовность), EVALUATOR_VERSION]), 16)
    rec = {
        'evaluation_id': ид,
        'evaluated_at': TODAY,
