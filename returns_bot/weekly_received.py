...
# Вывоз со стока FBO — тоже побывавший у покупателя товар, а не заводская упаковка.
# Сергей 10.08.2026: работаем только по ФБС, на склад Ozon товар попадает после отмены,
# невыкупа или возврата после получения. Поэтому проверяются ВСЕ коробки, «Брак» — в первую
# очередь. Прежняя формулировка «у покупателя не был» была неверной.
REMOVAL_NOTE = ("Работаем только по ФБС: на склад Ozon товар попадает после отмены, невыкупа "
                "или возврата после получения — вскрытым может быть любой. Проверять все, "
                "🔴 «Брак» — в первую очередь.")
...
def build(since, until):
    ...
    out.append(f"<b>📦 Вывоз со стока FBO Ozon — {len(removals)}, все на проверку</b>")
    out.append(REMOVAL_NOTE)
    for x in removals:
        brak = "брак" in (x["return_type"] or "").lower()
        out.append(f"• {'🔴 ' if brak else ''}заявка "
                   f"<code>{_esc(x['order_number']) or x['return_id']}</code> · "
