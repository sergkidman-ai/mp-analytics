...
def normalize(account, raw):
    ...
    head = {
        ...
        # ищется вообще — это ШК наклейки на коробке, его печатаем отдельной строкой.
        # У части старых закрытых строк orderId пуст (0) — тогда откатываемся на стикер.
        "order_number": str(raw.get("orderId") or raw.get("stickerId") or raw.get("shkId")),
