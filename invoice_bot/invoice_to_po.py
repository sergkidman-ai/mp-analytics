...
SUPPLIERS = {
    ...
    "9731107362": {"name": "Феррет",            "article": "name_regex",
                   "pattern": r"\b(?:CS|GG)-[0-9A-Za-z]+", "pdf": "ferret"},
...
def parse_table(rows):
    ...
    for r, row in enumerate(rows):
        ...
        if any(x in QTY_L for x in low) and "цена" in low and "№" in low:
            hdr = r; break
    if hdr is None:
    ...
    for c, v in enumerate(rows[hdr]):
        n = _norm(v)
        if n == "№" and "num" not in cols: cols["num"] = c
        ...
        elif n == "цена" and "price" not in cols: cols["price"] = c
        elif n == "сумма" and "sum" not in cols: cols["sum"] = c
    items = []
...
def parse_pdf_ferret(text):
    arts = re.findall(r"\b(?:CS|GG)-[0-9A-Za-z]+", text)
    rows = []
    for m in re.finditer(r"^\s*(\d+)\s+(\d+)\s+.*?(\d+,\d{2})\s+(\d+)\s+шт\s+(\d+,\d{2})\s*$", text, re.M):
