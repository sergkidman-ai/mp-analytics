...
def _parse_total(text):
    ...
    def pick(line, allow_int):
        ...
        nums = re.findall(r"\d[\d ]*[.,]\d{1,2}", line)              # суммы с копейками — приоритет (точнее).
        #   1–2 знака: calamine отдаёт денежный float без хвостового нуля («5583,70»→«5583.7»); жадный
        #   \d{1,2} берёт 2 знака, когда они есть, и ловит одинарный float-хвост (иначе целочисленный
        #   проход даст 5583 → мнимое расхождение с суммой строк и черновик вместо автопроводки).
