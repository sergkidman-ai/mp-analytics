...
DEFECT_RX = re.compile(
    ...
    r"не\s+подош|не\s+подход|не\s+совпад|не\s+определя|друг(ой|ое)\s+товар|мажет|мазал|"
    # 21.09.2026, п.3 пакета по неделе модерации: «не пропечатывает до конца» (Яндекс, 4★), «что-то
    # плохо работает», «один не тот» в наборе. «до конца» — только рядом с отрицанием или «плохо»:
    # голое «хватило до конца» — похвала, не дефект.
    r"не\s*пропечат|плохо\s+работа|(?:не|плохо)\s+\w+\s+до\s+конца|до\s+конца\s+не\b|"
    r"не\s+т(?:от|ой|у|е)\s+(?:цвет|картридж|модел|размер|оттен)|"
    r"(?:один|одна|одного|одной)\b[^.!?]{0,25}\bне\s+т(?:от|а|ой|у)\b", re.I)
...
# ─────────────────────── композитор ОТЗЫВА ───────────────────────
# Ситуация закрыта (10.09.2026): товара у покупателя уже нет — выкинул, разбил, вернул, купил
# другой. Отвечать шаблоном тут нельзя: и «встряхните картридж», и «напишите нам в чат» звучат
# издевательски. Уходит маркер на человека — тот же приём, что у домен-фильтра: маршрут human,
# текст с ⚠️, и публикация закрыта дважды (publish_gate режет и маршрут, и маркер).
CLOSED_MARKER = ("⚠️ " + neg_templates.CLOSED_NOTE + ": покупатель избавился от товара "
                 "(выкинул / разбил / вернул / купил другой). Шаблон не подставляем — решает человек.")
...
def draft_review(r, name, prod):
    ...
    if rating <= 3 or complaint:
        ...
        if _k == "closed":
            return "negative", draft, "human", 0.0, "neg_type:closed"
        if empty:
