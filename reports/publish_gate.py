...
def verdict_full(row, text=None):
    ...
    if claim and not dead and not _safe_handoff(g, own_draft, body):
        # п.2 пакета 21.09.2026: на отзыве ≥4★ кнопка «✅ как есть» доступна независимо от класса —
        # причина та же, но формулировка другая и в NO_OVERRIDE не входит: решает оператор.
        if kind == "review" and (row.get("rating") or 0) >= 4:
            reasons.append("претензия на ≥4★ — проверьте, можно отправить как есть")
        else:
    ...
    elif kind == "question" and source == "модель+карточка":
        # п.11 (21.09.2026): модель сама сказала, что добавила своё знание к карточке.
        reasons.append("источник — карточка плюс знание модели: часть фактов не из карточки")
