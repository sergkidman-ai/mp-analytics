...
def verdict(row, text=None):
    ...
    if g.get("qa_guard"):
        ...
        if eased:
            # ВАЖНО: снимаем только пометку «полярность не определена» — там guard ничего не менял.
            # Вариант без этой оговорки означает, что guard ВПИСАЛ в текст «Да,»/«Нет,» сам, и на
            # замере 10.09 это ломало смысл ответа («Нет, технически заправить можно»). Публиковать
            # текст, испорченный нашим же guard'ом, нельзя ни при каком классе.
            qa = [x for x in qa if "полярность не определена" not in str(x)]
        if qa:
