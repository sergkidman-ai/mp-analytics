...
def _quality_guard(reply, question):
    ...
    if out and _YESNO_Q_RX.search(question or ""):
        ...
        if not _DIRECT_RX.match(first):
            # «Да» ставим только при ОДНОЗНАЧНО положительной первой фразе. Инцидент на mid=19:
            # «совместим ТОЛЬКО с M2000DW» — про модель покупателя это «нет», а по слову «совместим»
            # автоподстановка выдала бы «Да». Оговорки (только/лишь/не заявлен/уточните) → полярность
            # не определена: помечаем нарушение модератору, но текст не трогаем.
            ...
            if head:
                out[i] = f"{head}, {first[0].lower() + first[1:]}"
            viol.append("не прямой ответ" + ("" if head else " (полярность не определена)"))
