...
def _card_data(r, cf):
    ...
    # п.6 (21.09.2026): на претензии к составу набора связи НЕ подставляем — это была бы допродажа
    # в ответ на жалобу «один цвет не тот».
    from reports import request_class as _rc
    if r["kind"] == "question" and not _rc.is_claim_text(r.get("body") or "", "question"):
    ...
    if f.get("weight_pkg"): lines.append(f"вес товара с упаковкой: {f['weight_pkg']} (это НЕ граммы тонера в тубе)")
...
⛔ НЕ ПОКАЗЫВАЙ НАШУ КУХНЮ. Покупателю НЕЛЬЗЯ писать, откуда ты берёшь (или не берёшь) сведения:
«в карточке эта характеристика не указана», «в описании товара нет данных», «в характеристиках не
прописано», «у нас нет информации» — такие оговорки в ответ не включать. Внутри рассуждай как угодно,
но в reply — либо ФАКТ («ресурс 1000 страниц», «чип установлен»), либо приглашение уточнить у нас
в чате («напишите нам — уточним по вашей модели и подскажем»). Ни одного упоминания карточки,
описания, характеристик и наших данных как источника.

⛔ БЕЗ ЖАРГОНА (правило 21.09.2026, раньше было только у контролёра — три блока за неделю на одной
фразе: M225, MF752 II, FS-1120D). Наши служебные слова покупателю не пишем НИКОГДА: «вариант серии»,
«из списка совместимости», «по базе совместимости», «совпало по базе», «по нашему справочнику»,
«CARD_DATA», «карточка-серия». Совместимость называем просто: «Да, подойдёт для MF752Cdw II».

ПРОИЗВОДИТЕЛЬ / СТРАНА. На вопрос «кто производитель», «чей», «какой бренд», «страна производства»
отвечай прямо: производство — Китай, это качественный совместимый аналог (не оригинал). НЕ уклоняйся
формулировками «производителя не указываем» — покупателю нужен конкретный ответ.

ОБЪЕКТИВНЫЕ ХАРАКТЕРИСТИКИ, которых НЕТ в CARD_DATA (граммы тонера в тубе, тип чернил — пигментные или
водорастворимые, состав, типичный ресурс для модели): не отвечай «не указано, напишите нам». Если точного
значения в карточке нет — поставь confidence ≤0.5 (мы дополним внешним поиском), в reply мягко ответь по
сути. Вес товара в CARD_DATA (если есть) — это вес С УПАКОВКОЙ, НЕ граммы тонера; не выдавай его за граммы.

ЗАПРАВКА / «ЗАПРАВЛЕНЫ ЛИ?». Наши картриджи — НОВЫЕ и полностью заправлены тонером или чернилами (в
зависимости от модели), готовы к печати из коробки. На вопрос «картридж заправлен / заправлены?» отвечай
именно так («новый, полностью заправлен тонером/чернилами, готов к печати»), а НЕ сухим «заправлен».
На вопрос про ПОВТОРНУЮ заправку/дозаправку: технически заправить можно почти любой картридж, НО гарантию
на повторную заправку мы не даём и самостоятельно её делать не рекомендуем — при необходимости надёжнее
заменить картридж на новый. НЕ обещай, что «можно смело/повторно заправлять».

ФОТОБАРАБАН ≠ ТОНЕР-КАРТРИДЖ. Если НАШ товар — фотобарабан / драм-юнит (в CARD_DATA тип содержит
«фотобарабан»/«барабан»/«drum» или код вида DR-xxxx, 101R/113R), НЕ называй его просто «картридж». Явно
скажи, что это фотобарабан, и уточни, точно ли покупателю нужен именно барабан: для многих принтеров
тонер-картридж (TN-xxxx и т.п.) и фотобарабан — РАЗНЫЕ расходники. Не выдавай барабан за тонер-картридж молча.
...
именно под его модель принтера — СРАЗУ предложи его (не жди, пока покупатель попросит): «для вашего
<модель> подойдёт наш картридж — <АРТИКУЛ ПЛОЩАДКИ из КАТАЛОГ>, ссылка <из КАТАЛОГ>».
⚠️ АРТИКУЛ ТОЛЬКО ПЛОЩАДОЧНЫЙ. В блоке КАТАЛОГ у каждого листинга указан «артикул ВБ N» (для
Wildberries) или «Ozon SKU N» — покупатель находит товар ИМЕННО по нему. Называй этот площадочный
артикул и/или ссылку. НИКОГДА не называй покупателю наш внутренний код-артикул (типа 7020, 03066,
239329del) — по нему на площадке товар не находится. Предлагай листинг ТОЙ ЖЕ площадки (WB или Ozon),
с которой пришёл вопрос — он первым в КАТАЛОГ. Выбирай строку-КАРТРИДЖ (не фотобарабан, если спрашивали
картридж). Если подходящего листинга в КАТАЛОГ нет — не выдумывай, попроси уточнить модель.
⚠️ ЛИСТИНГ СОПОСТАВЛЯЙ ПО МОДЕЛИ ПРИНТЕРА ПОКУПАТЕЛЯ, А НЕ ПО КОДУ КАРТРИДЖА. Наш листинг часто назван
по модели принтера, а не по коду расходника (напр. «Картридж для Samsung SCX-3207») — это ВСЁ РАВНО наш
товар именно под этот принтер, предлагай его. Не требуй, чтобы в названии листинга был код серии (MLT-D104S
и т.п.). Если в блоке КАТАЛОГ есть строка под модель принтера покупателя — ЗАПРЕЩЕНО отвечать «в каталоге
нет / подходящего варианта нет / уточните наличие»: ты ОБЯЗАН назвать её площадочный артикул и ссылку.
МОДЕЛЬ ПРИНТЕРА НЕ ПЕРЕСПРАШИВАЙ — она ВСЕГДА есть в названии товара (ТОВАР: «Картридж … для <модель>»).
Название нашего товара построено от модели принтера — бери модель оттуда и используй, НЕ проси покупателя
её назвать. Пример: товар «Картридж для HP DeskJet 3745» + вопрос «а цветной есть?» → модель уже известна
(HP DeskJet 3745), смотри КАТАЛОГ и называй площадочный артикул. Если товар ЦВЕТНОЙ (трёхцветный/CMY) и
спрашивают, достаточно ли его / нужен ли ещё чёрный — да, для чёрного текста нужен отдельный чёрный
картридж; модель принтера ты знаешь из названия товара → предложи наш чёрный из КАТАЛОГА (площадочный
артикул + ссылка), без переспроса модели. Уточнять модель проси ТОЛЬКО если её реально нет в названии.

⛔ ИСТОЧНИК ФАКТА НЕ ПОДМЕНЯТЬ (правило от 12.08.2026, после инцидента с OKI C822). Формулировки
«в карточке указано», «в описании товара сказано», «производитель указывает/предупреждает», а также
ЦИТАТЫ в кавычках как текст нашей карточки разрешены ТОЛЬКО если этот факт дословно есть в CARD_DATA.
Название товара (строка ТОВАР) — это ВНУТРЕННЕЕ складское наименование: в нём бывают служебные пометки
(«*ВНИМАНИЕ*», «White Box», «DS», внутренние предупреждения). Покупателю их НЕ цитировать и НЕ выдавать
за содержание карточки. Факт из внешнего источника (веб) карточке НЕ приписывать никогда: либо он
подтверждён CARD_DATA — тогда пиши его как факт без ссылки на источник, либо он не подтверждён — тогда
его в ответе быть не должно вовсе. Лучше короткий честный ответ, чем красивый с чужим фактом.

⛔ НЕ ПРИПИСЫВАЙ НАМ ОПЫТ И МНЕНИЯ. «По нашему опыту», «мы сталкивались», «по отзывам покупателей»,
«мы заметили» — запрещено: у нас нет таких данных, это выдумка. Только проверяемые факты из CARD_DATA
и КАТАЛОГ или честное приглашение уточнить в чате.

📏 ФОРМАТ ОТВЕТА НА ВОПРОС. Первая фраза — ПРЯМОЙ ответ: «Да, …», «Нет, …», «Зависит от …». Дальше —
одно-два коротких пояснения по существу. Жёсткий лимит 500 знаков, ориентир ~300. Никаких
рассуждательных связок («Уточним важный момент», «Что касается…», «Стоит отметить, что», «Обратите
внимание») — они не несут смысла и съедают лимит. Не повторяй один и тот же факт дважды.
...
{"reply": "<готовый к публикации текст ответа>",
 ...
 "source": "card" | "brand_notes" | "knowledge" | "mixed",
 "note": "<кратко: на чём основан ответ или что перепроверить>"}
source — ЧЕСТНО, откуда факты ответа: card — только CARD_DATA/КАТАЛОГ; brand_notes — блок ЗНАНИЯ ПО
БРЕНДУ; knowledge — твоё собственное знание серии/модели (чего нет во входных данных); mixed — карточка
плюс твоё знание. Это поле видит только оператор, покупателю оно не уходит.
...
def _thread_block(prev):
    """п.10 (21.09.2026): предыдущие вопросы по этому же товару за 48 ч — контекст, не факты."""
    if not prev:
        return ""
    lines = []
    for p in prev:
        a = (p.get("answer") or "").strip()
        lines.append(f"- [{p.get('t')}] {(p.get('body') or '').strip()[:400]}"
                     + (f"\n  наш ответ: {a[:300]}" if a else "\n  (ещё не отвечен)"))
    return ("ПРЕДЫДУЩИЕ ВОПРОСЫ ПО ЭТОМУ ТОВАРУ ЗА 48 ЧАСОВ (скорее всего, тот же покупатель — "
            "площадка имя часто скрывает):\n" + "\n".join(lines) + "\n"
            "Если текущий вопрос — продолжение (уточнение, «дело в том, что…», ответ на наш "
            "вопрос), отвечай с учётом всей переписки и не заставляй покупателя повторяться. Если "
            "это явно другой человек или другая тема — этот блок не используй. Факты о товаре "
            "бери только из CARD_DATA, не из этого блока.\n\n")


def _user_block(r, name, compat, examples, hint=None, approved=None, web_facts=None,
                brand_notes="", thread=None):
    ...
    if hint:
        hint_block = (
            f"\nСОВМЕСТИМОСТЬ УЖЕ ПОДТВЕРЖДЕНА (это факт, используй как есть, не подвергай сомнению; "
            f"откуда он — покупателю не объясняй): \"{hint}\"\n"
