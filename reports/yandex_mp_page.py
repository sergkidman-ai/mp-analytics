...
def build(acc):
    ...
    rev = L["revenue"]; sub = L["subsidy"]; nett = L["netting"]
    # ОБОРОТ-ГРОСС = оплата покупателя + зачёт баллами Маркета (обе части из актов) — та же форма,
    # что «наша цена» у Ozon и ВБ. Удержания в L уже начисленные (см. balance() в yandex_mp_report),
    # поэтому «Итого к перечислению» ниже не меняется: зачёт сокращается между оборотом и услугами.
    ob = [rev[i] + nett[i] for i in range(N)]
    ...
    itog = [sum(L[k][i] for k in MP_EXP) for i in range(N)]      # Итого удержания = услуги (БЕЗ возвратов)
    payout = [ob[i] - itog[i] - ret_money[i] for i in range(N)]  # к перечислению = Оборот − услуги − возвраты ≈ ЛК
    itog_pct = [(itog[i] / ob[i] * 100 if ob[i] else 0) for i in range(N)]
    # начисленные услуги минус зачёт баллами = то, что Маркет реально снял с денег покупателя
    itog_money = [itog[i] - nett[i] for i in range(N)]
    ...
    H.append(row("Возвраты, шт (товар + невыкуп)", retc, "count_dn", ob, k="returns_cnt"))
    H.append(row("Средний чек, ₽", check, "check", ob, tag="расчёт", k="check"))
    H.append(sect("Продажи (оборот)"))
    H.append(row("Продажи — оборот (наша цена)", ob, "inflow", ob, sect_pct="100.0%", tag="расчёт", k="own"))
    H.append(row("Выручка (деньги покупателя)", rev, "inflow", ob, sub=True, k="revenue"))
    H.append(row("Зачёт баллами Маркета (за его счёт)", nett, "inflow", ob, sub=True, showpc=True, k="netting"))
    H.append(row("Субсидия по заказам", sub, "inflow", ob, showpc=True, tag="справка", k="subsidy"))
    H.append(sect("Удержания площадки (начислено)"))
    ...
    H.append(row("Продвижение (буст + полки + лояльность + договор)", L["promotion"], "expense", ob, showpc=True, k="promotion"))
    ...
    H.append(row("Программа лояльности и отзывы", L["reviews_cost"], "expense", ob, sub=True, k="reviews_cost"))
    H.append(row("Реклама по договору на размещение 354817/19", L["ad_contract"], "expense", ob, sub=True, k="ad_contract"))
    ...
    H.append(row("в т.ч. удержано из выручки (деньгами)", itog_money, "expense", ob,
                 sub=True, showpc=True, tag="расчёт", k="itog_money"))
    H.append(row("в т.ч. погашено баллами Маркета (не деньгами)", nett, "inflow", ob,
                 sub=True, showpc=True, tag="справка", k="netting"))
...
SUB = ('Данные из <b>витрины Яндекс.Маркета</b> (Партнёр-API: заказы + отчёт услуг + закрытие '
       'месяца) + себестоимость из МойСклад. <b>Оборот = наша цена</b> = деньги покупателя + '
       '<b>зачёт баллами Маркета</b> (скидку покупателю Маркет даёт за свой счёт и гасит ею свои же '
       'услуги) — от него считаются все % и подсветка. Та же форма, что на вкладках Ozon (баллы) и '
       'ВБ (СПП): сначала общая продажа, под ней — из чего состоит. <b>Удержания показаны '
       'НАЧИСЛЕННЫМИ</b>, под итогом две справки — сколько удержано деньгами и сколько погашено баллами; на «Итого к '
       'перечислению» и чистую это не влияет (зачёт сокращается). <b>«Субсидия по заказам» — '
       'справочно</b>: та же скидка, но посчитанная по месяцу ЗАКАЗА, а не по акту (лаг). '
       'Строки — водопад: оборот → минус комиссия, логистика, перевод, продвижение (буст + полки + '
       'лояльность), агентское, прочие, подписка → <b>Итого удержания (услуги)</b> → минус возвраты → '
       '<b>Итого к перечислению</b> (сверено с ЛК «Подлежит перечислению на счёт Заказчика») → '
       'минус COGS → Чистая. '
...
FOOT = ('Все строки — из витрины <b>Яндекс.Маркета</b> (Партнёр-API). <b>Оборот</b> = Оплата '
        'покупателя (payment) + <b>Зачёт баллами Маркета</b> (NETTING отчёта о стоимости услуг): '
        'субсидию Маркет не перечисляет деньгами, а гасит ею собственные услуги, поэтому в отчёте '
        'услуги видны уже нетто. Обе части взяты из АКТОВ, поэтому «Итого к перечислению» не '
        'изменилось ни на рубль: (оплата + зачёт) − (услуги + зачёт) = оплата − услуги. '
        '<b>Строки удержаний — начисленные</b> (деньгами + зачёт), под итогом две '
        'справочные строки разбивают их на части: <b>«в т.ч. удержано из выручки '
        '(деньгами)»</b> = начислено − зачёт (июль-2026: 1 568 764 − 935 413 = 633 351 ₽ — '
        'ровно столько Маркет снял с денег покупателя) и <b>«в т.ч. погашено баллами»</b>. <b>«Субсидия по заказам» (subsidy)</b> — та же '
        'скидка со стороны заказов: живёт в месяце ЗАКАЗА, а акт приходит с лагом (июнь-2026: зачёт '
        '909 тыс при субсидии 730 тыс, июль: 935 тыс при 1 000 тыс), поэтому она справочная и в '
        'расчёт не берётся. '
        '<b>«Итого удержания Маркета»</b> = комиссия + логистика + перевод/эквайринг + '
        'продвижение (буст-продажи + буст-показы + полки + программа лояльности и отзывы) + '
        'агентское + прочие + подписка (<b>без возвратов</b>). <b>«Итого к перечислению»</b> = '
        'Оборот − Итого удержания − Возвраты = ЛК «Подлежит перечислению на счёт Заказчика» '
        '(июнь-2026 ≈ 758к, Δ к ЛК &lt;0.5%). Себестоимость — '
        ...
        '<b>«Возвраты, шт»</b> = физический возврат товара на склад: товарные возвраты (RETURNED) + '
        'невыкупы (покупатель не забрал — товар вернулся на сток), по <b>месяцу возврата</b> '
        '(сверено с МС salesreturn, июнь ≈44 ≈ МС 39). Деньги «Возвраты (удержано)» — из закрытия ЛК '
        '(«Возвращено потребителям»), охватывают все денежные возвраты покупателям и уже включают '
        'невыкупы — счётчик (склад) и деньги (касса) намеренно в разных осях. '
