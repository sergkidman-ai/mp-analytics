...
def read_file(path):
    ...
    # (у обычных акций «Товары для акции_…»). Бот добавляет спереди «дата_время_Имя_логин_»
    ...
    m = re.search(r"(?:Все товары подходящие для акции|Товары для акции)\s*(.+)$", base)
    m = (m.group(1) if m else base)
    m = re.sub(r"\s*\d{2}\.\d{2}\.\d{4}.*$", "", m).strip(" _")
