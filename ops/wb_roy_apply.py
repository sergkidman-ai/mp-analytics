...
Решение Сергея 17.08.2026 по неделе 10–16.08:
  ...
  🟡 жёлтый   → не трогаем
  ⚫ вывод → ставка на пол (с --blackout); САМО СНЯТИЕ с рекламы — в ops/wb_roy_evict.py,
              отдельным шагом с подтверждением человека. ⬜ маржи нет → не трогаем.
...
def main():
    ...
    if a.notify:
        ...
        send(f"*Рой ВБ — {pathlib.Path(a.csv_path).stem}*\n"
             ...
             f"Ставки применены автоматом: 🟢 +10 %, 🔴 −10 %, 🟤/⚫ на пол.\n"
             f"Ручного шага здесь нет; человек нужен только на выводе ⚫ из кампаний "
             f"(отдельное сообщение со ссылкой).")
