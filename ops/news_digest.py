...
# DeepSeek v4 Pro — думающая модель: сначала тратит output-токены на размышление, потом
# отдаёт JSON. Замер на письме Маркета 21.08: thinking 17.4 тыс. знаков, текст 1.3 тыс.,
# итого 5518 выходных токенов. На лимите 700 и 3000 весь лимит уходил в thinking, текстового
# блока не было вовсе (stop_reason='max_tokens') — молчаливая потеря денег: заплачено, ответа нет.
MAX_OUT = int(os.environ.get("EV_DIGEST_MAX_TOKENS", "8000"))
AVG_OUT = 5500         # факт замера: размышление съедает в 6 раз больше, чем сам JSON
...
- Наш товар: совместимые картриджи и расходные материалы для оргтехники. Категория на
  Яндекс.Маркете — «Расходные материалы для оргтехники». Если новость называет категории,
  прямо скажи, попадает наша или нет; если не попадает — так и напиши, не притягивай.
...
def _parse(text):
    ...
    if not m:
        raise ValueError("модель вернула не JSON (пусто — вероятно, лимит съело размышление)")
...
def estimate(rows):
    ...
    tok_out = len(rows) * AVG_OUT                    # с учётом thinking-токенов
