...
ТОЛЬКО ПЛАТЕЖИ ИЗ ВЫПИСКИ. Разбираем то, что записал наш конвейер (`from_bank`, признак —
`syncId`), И ручные документы, заведённые уже при живом банковском контуре. Не трогаем ЛЕГАСИ —
авансы, заведённые в МС руками ДО подключения API (`legacy_manual`, граница `ALFA_MS_SINCE`):
их не привязываем никуда и в баланс поставщика не берём (статус `manual-skip`), остатки владелец
разносит сам.
...
MS_SINCE = os.getenv("ALFA_MS_SINCE") or ""      # с этой даты выписку в МС пишет наш контур


def legacy_manual(payment):
    """Ручной аванс ЛЕГАСИ — заведённый до того, как выписку начал писать банковский контур.

    Гейт по ДАТЕ, а не по отсутствию `syncId`. Причина техническая: МойСклад запрещает
    проставлять `syncId` уже существующему документу (ошибка 1047), поэтому платёж, заведённый
    руками при живом контуре, ботовским сделать нельзя — по прежнему правилу он навсегда
    оставался бы «ручным» и не разнёсся бы ни сейчас, ни по будущим поставкам.

    Факт 04.09.2026: аванс Тонеропттторгу 100 000 ₽ от 31.08 (№610) потерялся из-за бага
    антидубля в `bank_ms`, Сергей завёл документ руками — и он попал ровно в эту ловушку.
    Замер тогда же: ручных документов у ЦК с 27.07 всего один, этот.

    Легаси до `ALFA_MS_SINCE` остаётся за человеком, как и решено 19.08.2026."""
    return (not from_bank(payment)
            and bool(MS_SINCE) and (payment.get("moment") or "")[:10] < MS_SINCE)
...
def link_payment(payment, apply=False):
    ...
    if advance:                                  # 2) аванс — FIFO по неоплаченным приёмкам
        ...
        if legacy_manual(payment):
            # ручной аванс ДО подключения банка: его разносил человек, мы в него не лезем
            return "manual-skip", (f"аванс заведён в МС руками до {MS_SINCE} — не привязываем")
