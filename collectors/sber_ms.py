...
* **Привязка платежей к приёмкам ВЫКЛЮЧЕНА.** `alfa_link` разбирает назначение платежа по
  мостам, написанным под поставщиков Цифрового Квадрата (Феррет: № счёта = № приёмки;
  КВК: номера заказов). Для поставщиков Дисквэра таких мостов ещё нет — привязывать вслепую
  нельзя, деньги сядут не на ту приёмку. Открытый вопрос 5 в `docs/BRIEF_INV.md`.
...
def sync(normalized, apply=False):
    """Выписка Сбера → МС. Без привязки к приёмкам (link_fn=None) — см. докстринг модуля."""
