...
def _csv_cat(fname, service_name):
    ...
    if "полк" in sn:                              # Полки — где бы ни лежали (shelf.csv и пр.)
        return "shelf"
    if "средн" in sn or "миля" in sn:             # доставка «средняя миля» — отдельный вид логистики
        return "logistics"
...
def _csv_cost(fname, rec):
    ...
    Реклама (буст/баннеры): берём SERVICE_PRICE = «Стоимость услуги» (плата ПОСЛЕ софинансирования
    Яндекса), а НЕ PREPAID/POSTPAID/PAYMENT — те несут валовую стоимость заказа/оборот буста и
    завышают продвижение в разы (июнь: буст-продаж gross 125947 vs услуга 5132; ЛК = услуга)."""
    if fname == "placement.csv":                   # комиссия за размещение — реальные деньги в TOTAL_AMOUNT
        return _fnum(rec.get("TOTAL_AMOUNT"))
    # boost/cpm-boost/product-banners + delivery/express/payment_*/processing/storage/отзывы/подписка/
    # средняя миля → SERVICE_PRICE (фолбэк AMOUNT_WITHOUT_BONUSES/TOTAL_AMOUNT)
...
# Реклама (буст/баннеры/полки): реальная плата — колонка «Стоимость услуги, ₽» (= Постоплата −
# «Скидка за участие в совместных акциях», т.е. ПОСЛЕ софинансирования Яндекса). Колонки
# «Предоплата/Постоплата/Оплата, ₽» — это ВАЛОВАЯ стоимость заказа/оборот буста (в разы больше,
# сверено с ЛК июнь: буст-продаж Постоплата 125946.69 − скидка 120814.26 = услуга 5132.43; ЛК
# «Продвижение» = именно услуга). Логистика «средняя миля» — отдельный лист (был не в маппинге →
# терялся): июнь 75192.59, сверено с ЛК. Комиссия/размещение: имя листа менялось («…и услуг» →
# «…на витрине»).
SHEETS = {
    "Буст продаж, оплата за показы":  ("boost_shows",  ["Стоимость услуги, ₽"],       "Оплата бонусами", DATE),
    "Буст продаж, оплата за продажи": ("boost_sales",  ["Стоимость услуги, ₽"],       "Оплата бонусами", DATE),
    "Полки":                          ("shelf",        ["Стоимость услуги, ₽"],       "Оплата бонусами", DATE),
    "Товарные баннеры":               ("boost_shows",  ["Стоимость услуги, ₽"],       "Оплата бонусами", DATE),
    ...
    "Размещение товаров на витрине":   ("commission",   ["Стоимость услуги (AX*"],     None,              DATE_DT),
    "Доставка покупателю":            ("logistics",    ["Стоимость услуги, ₽"],       None,              DATE_DT),
    "Доставка (средняя миля)":        ("logistics",    ["Стоимость услуги, ₽"],       None,              DATE_DT),
...
def main():
    """Ночной шаг: сперва добираем свежие месяцы из API, ПОТОМ накатываем ручной файл ЛК —
    файл перекрывает API за покрытые им месяцы (у API нет «средней мили» и завышен буст, см.
    _csv_cost/SHEETS). Идемпотентно помесячно: месяцем владеет тот, кто записал последним."""
    collect_api()
    import_file()
