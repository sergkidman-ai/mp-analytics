...
def _agent_key(inn, name):
    """Кто получатель — для антидубля. ИНН, а без него нормализованное имя.

    ИНН есть у всех, кого мы вообще заводим (физлица отсекает `skip_reason`), поэтому имя —
    аварийный запасной ключ, а не рабочий: написание в выписке и в карточке МС расходится
    («ОБЩЕСТВО С ОГРАНИЧЕННОЙ ОТВЕТСТВЕННОСТЬЮ "ТОНЕРОПТТОРГ"» против «ТОНЕРОПТТОРГ (Изипринт)»)."""
    return (inn or "").strip() or _norm(name)
...
def existing_index(typ, day, org_id=None):
    ...
    Поэтому перед записью сверяемся ещё и «по-человечески»: получатель + сумма + номер
    банковского документа + день. Ключ `sync` — наши собственные документы, их PUT просто обновит.

    ПОЛУЧАТЕЛЬ В КЛЮЧЕ — не украшение, ровно та же природа, что и организация ниже. Без него
    два РАЗНЫХ поставщика, получивших в один день одинаковую сумму, схлопывались в один
    документ, и второй платёж молча не заводился («ошибок 0» в логе). Факт 31.08.2026:
    Цифровой Квадрат заплатил по 100 000 ₽ Позитиву и Тонеропттторгу — Позитив записался
    в 13:07, Тонеропттторг в 13:37 был признан его дублем и потерян; аванс повис в очереди
    платёжек в статусе «в банке», а баланс поставщика в МС занижен на 100 000 ₽.
    Замер по выписке июль–август 2026: так терялось 6 платежей.
    ...
    idx = {"by_num": {}, "by_sum": {}, "sync": set(), "used": set(), "total": 0}
    offset = 0
    while True:
        # expand=agent — ради ИНН получателя: он входит в ключ антидубля, а в неразвёрнутой
        # строке лежит только meta-ссылка, по которой пришлось бы ходить за каждой карточкой.
        r = get(f"/entity/{typ}?filter={flt}&limit=100&offset={offset}&expand=agent")
        rows = r.get("rows", [])
        for d in rows:
            s = d.get("sum")
            ag = d.get("agent") or {}
            who = _agent_key(ag.get("inn"), ag.get("name"))
            if d.get("syncId"):
                idx["sync"].add(d["syncId"])
            for num in {str(d.get("name") or "").strip(),
                        str(d.get("incomingNumber") or "").strip()}:
                if num:
                    idx["by_num"].setdefault((who, s, num), []).append(d)
            idx["by_sum"].setdefault((who, s), []).append(d)
        idx["total"] += len(rows)
        if len(rows) < 100:
            break
        offset += 100
...
def find_existing(idx, op, sync_id):
    ...
    def take(docs):
        """Первый ЕЩЁ НЕ ЗАНЯТЫЙ документ из списка — и сразу помечаем занятым.

        Один документ МС закрывает ровно одну операцию выписки. Без этого два одинаковых
        платежа одному получателю в один день (факт 20.08.2026: два раза по 49,00 ₽ Озон-банку)
        оба смотрели на одну и ту же строку в МС, и второй молча терялся."""
        for d in docs or ():
            if id(d) not in idx["used"]:
                idx["used"].add(id(d))
                return d
        return None

    s = _kop(op.get("amount"))
    who = _agent_key(op.get("counterparty_inn"), op.get("counterparty_name"))
    num = str(op.get("document_number") or "").strip()
    if num:
        hit = take(idx["by_num"].get((who, s, num)))
        if hit is not None:
            return hit, "получатель+номер+сумма"
    hit = take(idx["by_sum"].get((who, s)))
    if hit is not None:
        # Номер не сошёлся, но у ЭТОГО получателя сумма за тот же день уже есть. Осознанно
        # считаем дублем: пропущенный платёж виден при сверке, задвоенный — портит учёт молча.
        # Разные получатели с одинаковой суммой дублем больше не считаются (см. existing_index).
        return hit, "получатель+сумма+день"
    return None, ""
